...
import { throwErr } from '@/config/mUtils' //utils 捕捉服务端http状态码的方法
import store from '@/store'   //引入vuex的相关操作
import router from '@/router'
import { baseUrl,baseImgPath,qwUrl} from '@/config/env';
...
export default function request(method, url, data) {  //暴露 request 给我们好API 管理
  ...
  if (method === 'post') {
    return axios.post(baseUrl+url, data)    //axios的post 默认转化为json格式
  } else if (method === 'get') {     
