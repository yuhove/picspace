<template>
  <div class="contentbox">
    ...
    <div class="specailbtn">
      <el-button type="primary" @click="changerouters" icon=" el-icon-plus" size="small">添加一级页面</el-button>
    </div>
    <div class="table_container flexbox">
      ...
      <div class="classifybox">
        <div>
          <el-button type="primary" @click="dialognewform=true" v-show="JSON.stringify(checkinfo) != '{}'">新增子页面</el-button>
        </div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="页面类型">
            <template slot-scope="scope">
              <p v-if="scope.row.isShow==0">目录页面</p>
              <p v-else>衍生页面</p>
            </template>
          </el-table-column>
          <el-table-column
      ...
      <el-dialog :title="title[type]" :visible.sync="dialognewform" class="specaildialog" :before-close="changetype" width="30%">
        <el-form :model="categoryOptions">
          <el-form-item label="页面名称：" label-width="100px">
            <el-input v-model.trim="categoryOptions.name" auto-complete="off" maxlength="20"></el-input>
          </el-form-item>
          <el-form-item label="页面router：" label-width="100px">
            <el-input v-model.trim="categoryOptions.url" auto-complete="off"  maxlength="250"></el-input>
          </el-form-item>
          <el-form-item label="页面类型：" label-width="100px">
            <el-switch
              v-model="categoryOptions.isShow"
              active-text="目录页面"
              inactive-text="衍生页面">
            </el-switch>
          </el-form-item>
...
<script>
  ...
  import {addProductCatalog,deleteProductCatalog,updateProductCatalog,getAllCatalog,upImg,addMenu} from '@/api/getDatelist'
  ...
  export default {
    ...
    methods: {
      ...
      async addMenu(){
        ...
        const res = await addMenu(this.categoryOptions);
        if(res.code==0){
          this.$message({
            type: 'success',
            message:'页面添加成功'
      ...
      handleNodeClick(data) {
        ...
        this.tableData=this.proJSON(this.protree,data.value)
